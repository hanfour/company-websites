...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  if (!caseData || caseData.type !== 'history') {
    return {};
  }
  ...
  const description = cleanSubText || cleanCaption || `建林工業 - ${caseData.name}，歷年建築營造實績`;
  ...
  return {
    ...
    keywords: [caseData.name, '建林工業', '歷年個案', '建築實績', '營造案例', '台北建案'],
    openGraph: {
    ...
    alternates: {
      canonical: `https://www.jianlin.com.tw/history/${id}`,
    },
  };
}
...
export default async function HistoryDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const jsonLd = {
    ...
    url: `https://www.jianlin.com.tw/history/${caseData.numberID}`,
    image: caseData.slider?.map(img => img.location || `${CDN_LINK}/${img.src}`) || [],
    author: {
      '@type': 'Organization',
      name: '建林工業股份有限公司',
    },
    datePublished: caseData.data_uploader || caseData.createdAt,
  };
