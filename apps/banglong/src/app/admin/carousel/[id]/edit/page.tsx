...
export default function EditCarousel() {
  ...
  const fetchCarousel = async () => {
    ...
    try {
      ...
      setTextPosition(item.textPosition || 'center');
      setTextDirection(item.textDirection || 'horizontal');
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        try {
          ...
          if (uploadResponse.ok) {
            ...
            if (errorData.fallback) {
              // 保留原始圖片
              console.warn('使用原始圖片URL');
            } else {
              throw new Error(errorData.error || '圖片上傳失敗');
            }
          }
        } catch (uploadError) {
          console.error('上傳圖片失敗，使用原始圖片:', uploadError);
        }
      ...
      if (!updateResponse.ok) {
        ...
        throw new Error(errorData.error || '更新輪播項目失敗');
      }
      
      // 成功後跳轉到輪播詳情頁面
      router.push(`/admin/carousel/${id}`);
      router.refresh();
    } catch (error) {
      console.error('Error updating carousel:', error);
      setError(error instanceof Error ? error.message : '更新輪播項目時發生錯誤');
    ...
    return (
      <AdminLayout>
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">編輯輪播項目</h1>
          </div>
          <Link
            href="/admin/carousel"
  ...
  return (
    <AdminLayout>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">編輯輪播項目</h1>
          <p className="text-gray-600 mt-1">修改輪播項目的內容和顯示設定</p>
        </div>
              ...
              <div>
                ...
                <div className="flex items-center space-x-4">
                  <label className="inline-flex items-center">
                    ...
                    />
                    <span className="ml-2 text-gray-700">啟用</span>
                  </label>
                  <label className="inline-flex items-center">
                    <input
                      type="radio"
                      ...
                      className="h-4 w-4 text-amber-600 border-gray-300 focus:ring-amber-500"
                    />
                    <span className="ml-2 text-gray-700">停用</span>
                  </label>
                </div>
              </div>
            </div>
